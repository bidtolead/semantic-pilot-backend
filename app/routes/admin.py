...
def get_all_users(authorization: str | None = Header(default=None)):
    ...
    for doc in users_ref:
        ...
        data.setdefault("promptTokens", 0)
        data.setdefault("completionTokens", 0)
        ...
        if data["promptTokens"] > 0 or data["completionTokens"] > 0:
...
# ----------------------------------------
# CLEANUP: ENFORCE 20-REPORT LIMIT
# ----------------------------------------
@router.post("/cleanup/enforce-history-limit")
def enforce_history_limit_all_users(authorization: str | None = Header(default=None)):
    """
    Retroactively enforce 20-report limit for all users.
    Deletes oldest reports beyond the 20 most recent for each user.
    ...
    try:
        ...
        for user_doc in users_snapshot:
            ...
            # Get all research documents for this user, ordered by createdAt desc
            research_ref = db.collection("users").document(user_id).collection("research")
            research_query = research_ref.order_by("createdAt", direction=firestore.Query.DESCENDING)
            research_docs = research_query.stream()
            
            research_list = list(research_docs)
            
            # If more than 20, delete oldest ones
            if len(research_list) > 20:
                docs_to_delete = research_list[20:]  # Get all docs beyond position 20
                for doc in docs_to_delete:
                    doc.reference.delete()
                    reports_deleted += len(docs_to_delete)
        ...
        return {
            ...
            "message": f"Enforced 20-report limit across {users_processed} users. Deleted {reports_deleted} old reports."
        }
    
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"Error enforcing history limit: {str(e)}"
        )
