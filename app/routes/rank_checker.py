...
def check_rank(payload: RankRequest):
    try:
        client = SerperClient()
        result = client.find_domain_rank(q=payload.query, domain=payload.domain, location=payload.location, max_results=20)
...
def batch_rank(payload: BatchRankRequest):
    try:
        ...
        for kw in payload.keywords:
            r = client.find_domain_rank(q=kw, domain=payload.domain, location=payload.location, max_results=20)
        ...
        return {"ok": True, "location": payload.location, "domain": payload.domain, "results": results}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
